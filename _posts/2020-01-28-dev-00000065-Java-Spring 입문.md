...
- https://mvnrepository.com/artifact/org.springframework/springloaded 버전확인 링크
- 적용전에 mvn spring-boot:run 을 꼭 한번 해줘야함(cmd) POM을 수정한 소스폴더 최상위 root에 디렉토리가 설정 되어야 한다.
...
- 다음은 Gradle을 이용해 Spring Loaded를 이용하는 방법이다.
  - https://gradle.org/releases/ 에서 다운로드 받은 후 로컬C에 압축해제 이 역시도 환경변수 를 설절 해주어야 함 bin폴더 타겟 해야함.
  - gradle <테스크 이름>
    - gradle tasks 연재 사용할 수 있는 task목록 확인
    - gradle -gui task gui로 확인하고 원하는 task를 더블클릭해서 실행하거나 자주 사용하는 테스크 목록이나 옵션 조합을 등록해두고 실행할 수도 있습니다.

## 기본 Gradle 설정

| 의존 구성   | 설명                                                                                  |
|-------------|---------------------------------------------------------------------------------------|
| compile     | 프로젝트 소스를 컴파일할 떄 필요한 의존 구성입니다.                                   |
| runtime     | 프로젝트를 컴파이하고 실행할 때 필요한 의존 구성으로, compile 구성을 포함합니다.      |
| testCompile | 프로젝트의 테스트 소스를 컴파일할 때 필요한 의존 구성으로, compile 구성을 포함합니다. |
| testRuntime | 테스트를 수행할 때 필요한 의존성으로, testCompile과 runtime 구성을 포함합니다.        |

```gradle
// 의존 라이브러리 선언 방법
// 의존구성 group: '그룹명', name: '모듈명', version: '버전'
// 의존구성 그룹명:모듈명:버전

buildscript {
	repositories() { // 빌드 스크립트에서 의존 라이브러리를 찾는 데 사용할 레파지토리 선언
		mavenCentral() // 메이븐의 레파지토리 사용
	}
	
	dependencies {
		// 스프링 부트 어플을 빌드하려면 스프링 부트 그레이들 플로그인이 필요합니다.
...
apply plugin: 'idea'
apply plugin: 'spring-boot' // 스프링 부트 지원 적용
...
// 프로젝트에서 의존 라이브러리를 찾는 데 사용할 레파지토리 선언
repositories {
	mavenCentral()
}

// 소스를 컴파일하고 빌드 완료된 프로그램을 실행할 때 사용할 자바 버전을 선언
sourceCompatibility = 1.8
targetCompatibility = 1.8

// 프로젝트에서 컴파일, 테스트 중에 사용할 의존 라이브러리를 선언
dependencies {
	compile("org.springframework.boot:spring-boot-starter-web")
	testCompile("org.springframework.boot:spring-boot-starter-test")
}

// 손쉬운 배포를 위해 생성할 그레이들 레퍼 버전을 선언
